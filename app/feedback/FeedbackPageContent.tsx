...
const phoneInputClassName =
  "w-full rounded-r-xl border border-base-300 bg-base-100 px-3 py-2.5 text-sm text-base-content shadow-sm outline-none transition focus:border-[#90AC19] focus:ring-2 focus:ring-[#90AC19]/20";
const phoneSelectClassName =
  "w-32 rounded-r-none border border-r-0 border-base-300 bg-base-100 px-2 py-2.5 text-sm text-base-content outline-none transition focus:border-[#90AC19] focus:ring-2 focus:ring-[#90AC19]/20";
...
export default function FeedbackPageContent() {
  ...
  return (
    <section className="surface-cream px-4 py-10 sm:px-6 lg:px-8 text-base-content">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="rounded-2xl border border-base-300 bg-base-100 p-5 shadow-sm sm:p-8">
          {isSubmitted && (
            <div className="mb-5 rounded-lg border border-green-300 bg-green-50 p-3 text-sm text-green-800 dark:border-green-500/40 dark:bg-green-500/10 dark:text-green-300">
              Thank you. Your response has been received.
            </div>
          )}

          {error && (
            <div className="mb-5 rounded-lg border border-red-300 bg-red-50 p-3 text-sm text-red-800 dark:border-red-500/40 dark:bg-red-500/10 dark:text-red-300">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            />
            {isLoadingServices && (
              <p className="text-sm text-base-content/60">Loading services...</p>
            )}
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                className="textarea textarea-bordered w-full bg-base-100 border-base-300 text-base-content focus:border-[#90AC19] focus:outline-none"
                placeholder="Tell us what would help your family most..."
              />
            </div>

            <label className="flex items-start gap-2 rounded-lg border border-base-300 bg-base-200/50 p-3 text-sm text-base-content/80">
              <input
                type="checkbox"
                checked={form.consent}
                onChange={(event) => updateField("consent", event.target.checked)}
                className="checkbox checkbox-sm mt-0.5 border-base-300 checked:border-[#90AC19] checked:bg-[#90AC19]"
              />
              <span>
            ...
            <button
              type="submit"
              className="btn w-full border-none bg-[#90AC19] text-white hover:bg-[#7e9717] disabled:bg-base-300 disabled:text-base-content/50"
